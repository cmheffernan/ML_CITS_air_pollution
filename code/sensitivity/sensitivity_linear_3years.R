...
for(i in seq(1,nrow(linear_settings))){
  ...
  if(linear_function=="counterfactuals"){
    ...
    if(linear_state=="pennsylvaniacomplete"){
      data<-data%>%
        filter(!monitor==4)
    }
    linear_state<-ifelse(linear_state=="pennsylvaniacomplete","pennsylvania",linear_state)
    ...
    dates<-as.data.frame(right_join(dates,expand.grid(State.Name=states,year=2017:2020)))
    ...
    save(all_validation,file = paste0("results/sensitivity/linear_models_3yrs/",linear_settings$State.Name[i],"_counterfactuals_",linear_pollutant,"_",linear_method,".RData"))
