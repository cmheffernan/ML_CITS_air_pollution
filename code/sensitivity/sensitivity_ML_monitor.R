...
cluster_settings$seed<-seq(1,nrow(cluster_settings))*23
...
cluster_LOO_monitor<-cluster_settings$LOO_monitor[i]
...
if(cluster_function=="counterfactuals"){
  ...
  if(!(data%>%filter(monitor==cluster_LOO_monitor,State.Name==cluster_state)%>%nrow())==0){
    data<-data%>%
      filter(!monitor==cluster_LOO_monitor)
    ...
    save(dfs_validation,file = paste0("results/sensitivity/ML_models_monitor/",cluster_state,"_counterfactuals_",
                                      cluster_pollutant,"_",cluster_method,"_",cluster_year,"_",cluster_LOO_monitor,"_df",".RData"))
